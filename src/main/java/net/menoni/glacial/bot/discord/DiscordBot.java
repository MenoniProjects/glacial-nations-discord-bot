...
import net.menoni.glacial.bot.service.MemberService;
...
public class DiscordBot {
    ...
    private final String glacialBotVersion;
    private final AutowireCapableBeanFactory autowireCapableBeanFactory;

    public DiscordBot(
            DiscordBotConfig config,
            AutowireCapableBeanFactory autowireCapableBeanFactory,
            String glacialBotVersion
    ) throws InterruptedException {
        this.config = config;
        this.autowireCapableBeanFactory = autowireCapableBeanFactory;
        this.glacialBotVersion = glacialBotVersion;
    ...
    private Activity factorActivity(DiscordBotConfig config) {
        if (config.getCustomBotStatus() == null || config.getCustomBotStatus().isEmpty()) {
            return Activity.playing("glacial-bot (" + this.glacialBotVersion + ")");
        } else {
        ...
        new Thread(() -> {
	        try {
                ...
                this.withGuild(g -> g.loadMembers().onSuccess(members -> {
                    ...
                    List<JdbcMember> botMembers = memberService.listMembers();
