...
public class DiscordLinkUtil {
	...
	public static boolean isDiscordChannelLink(String link) {
		if (!link.startsWith(CHANNEL_LINK_PREFIX)) {
			return false;
		}
		String idsPart = link.substring(CHANNEL_LINK_PREFIX.length());
		Matcher matcher = GUILD_CHANNEL_FORMAT.matcher(idsPart);
		if (!matcher.matches()) {
			return false;
		}
		return true;
	...
	public static String getDiscordChannelLinkChannelId(String link) {
		if (!isDiscordChannelLink(link)) {
			return null;
		}
		String idsPart = link.substring(CHANNEL_LINK_PREFIX.length());
		Matcher matcher = GUILD_CHANNEL_FORMAT.matcher(idsPart);
