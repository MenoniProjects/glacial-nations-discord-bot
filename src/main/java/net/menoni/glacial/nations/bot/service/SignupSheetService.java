...
public class SignupSheetService {
	...
	@Value("${glacial.signups.sheet.key:}")
	private String signupsSheetKey;
	@Value("${glacial.signups.sheet.tab:}")
	private String signupsSheetTab;
	...
	public List<String> runManualSignupSheetImport() throws IOException, CsvException {
		...
		List<String> resultLines = teamService.importCsv(parseCsvLines(lines));
	...
	private String getSheetCsvUrl() {
		...
		return "https://docs.google.com/spreadsheets/d/%s/export?format=csv&gid=%s".formatted(this.signupsSheetKey, this.signupsSheetTab);
	}
	...
	private List<ImportSignupsCommandHandler.SignupCSVLine> parseCsvLines(List<String[]> lines) {
		return lines.stream().map(line -> new ImportSignupsCommandHandler.SignupCSVLine(
				line[1],
				line[2],
				line[3],
				line[4],
